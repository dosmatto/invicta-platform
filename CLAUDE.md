...
- **Two versions, on purpose.** `APP_VERSION` is the **platform** (web). `APP_CAMPO_VERSION` is the **field app** (INVICTA Coleta, the `.aab`/`.ipa` in the stores) — it only moves when the field app itself changes, and it is what `preflight-android.mjs`, `preflight-ios.mjs` and `empacotar-android.mjs` read to derive `versionCode`/build. A platform-only change bumps `APP_VERSION` alone; never bump `APP_CAMPO_VERSION` just because the platform moved, or the stores get a "new version" with nothing new in it. The field app runs `3.x` (`3.0.0`, `3.1.0`, …), first digit only for a big change; always three parts, since the preflights can't parse `"3.0"`.
...
- **Entregar = commitar E dar push, sempre, ao terminar a tarefa.** Não pergunte, não deixe a correção parada na árvore de trabalho: o usuário testa o que está NO AR, e código não publicado é o mesmo que código não entregue. A pendência 38 foi corrigida e reportada como pronta duas vezes enquanto o `git log` parava numa versão anterior — o usuário testou a versão antiga e o defeito "continuava". Antes de encerrar: `npx tsc --noEmit`, `npm run build`, os `teste:*` do que foi tocado, e então commit + push.
- **Antes de commitar, confira se há trabalho em paralelo no mesmo projeto.** Várias sessões mexem neste repositório ao mesmo tempo. Rode `git status` e veja o que na árvore NÃO é seu; confira as outras sessões (list_sessions) e se alguma ainda está rodando neste `cwd`. Regras: nunca commite trabalho de outra sessão que ainda esteja em andamento; se o trabalho dela já estiver COMPLETO pelo padrão daqui (código + entrada no changelog + linha em `docs/PENDENCIAS.md`) e a sessão estiver parada, commite-o **em um commit separado e anterior ao seu**, com a versão dela — nunca misturado no mesmo commit. `version.ts` e `changelog.ts` são compartilhados: separe as entradas por versão em vez de arrastar a metade do outro.
...
Three planning methods selected in `components/talhao/AmostragemModulo.tsx` (plus a read-only **Registros de campo** tab): **Grid** (`SimuladorAmostragem`), **Zona de Manejo** (`SimuladorZonas`) and **Composta** (`SimuladorComposta`). The flow: cadastrar padrões → simular (live preview on map) → editar pontos → salvar grades → exportar / etiquetas.
- **Zona/Composta share one numbering model** (`src/lib/gradeZonas.ts`, modelo A = one composite sample per area): every point of area *k* carries `numero` *k* (what the lab gets) and `rotulo` `"k-seq"` (what the field sees). `numero` is the join back from the laudo — never put the text label in the numeric field. Manual point editing for both lives in `components/talhao/useEdicaoPontosZona.ts` + `EdicaoPontosBarra.tsx`; don't fork it.
- **Composta** (`src/lib/gradeComposta.ts`, pure, `npm run teste:gradecomposta`): N square cells of equivalent area clipped from the talhão (turf), guaranteed **exactly N with 100% coverage and no overlap** via a merge-slivers / split-largest pass, then M subsamples inside each cell (`selecionarPorMalha`). Cells are **deterministic** (no RNG — only the holes use the seed) and live on the GRADE (`GradeAmostragem.celulas`), not on the talhão: the field app reads them straight from `inv_grades`. From Fertilidade onward they replace the management zones via `src/lib/celulasDaGrade.ts` (`ehComposta` / `celulasComoZonaTalhao` / `celulasComoZonaGeom` / `bindingDasCelulas`) — the map is always constant-per-cell, never interpolated.
