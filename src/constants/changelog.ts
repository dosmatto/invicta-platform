...
export const CHANGELOG: Record<string, string[]> = {
  ...
  '2.175.0': [
    ...
    'A PLATAFORMA AGORA SE CURA SOZINHA DESSE ESTRAGO. As migrações antigas que ligavam o Fe e punham a ordem padrão rodam uma vez POR NAVEGADOR, mas o catálogo é um só para todos — quem já tinha rodado nunca mais olhava, e por isso a plataforma no ar não consertava nada. A nova cura roda em todo boot, sem marca no aparelho, e só age quando reconhece a assinatura do seed antigo: Fe desligado e sem o sinônimo "fe", ou Fe com ordem 100 ou mais enquanto pH, P e K estão todos abaixo de 20.',
    'AS DUAS MIGRAÇÕES ANTIGAS SAÍRAM, E ISSO TAMBÉM ACABA COM "OS PADRÕES MUDAM SOZINHOS". A que ligava o Fe e a que punha a ordem padrão marcavam no NAVEGADOR que já tinham rodado, mas gravavam no catálogo que é de todos: cada navegador novo ou com o cache limpo rodava de novo, regravava a ordem de fábrica por cima da ordem arrumada nas setas do Perfil e religava um Fe que o usuário tinha desligado. Agora a ordem das setas e o Fe desligado ficam como o usuário deixou, em qualquer aparelho. Os dois casos legítimos continuam cobertos: o catálogo do seed antigo é curado pela assinatura acima, e conta nova já nasce na ordem padrão (o seed de fábrica passou a gravar a ordem MO, pH, m%, V%… direto, com K%, Ca%, Mg% e CTCe nas posições deles). E conta nova já nasce com 2 casas nos micros (B, S, Zn, Cu, Mn e Fe), a mesma régua da pendência 43.',
    'O QUE A CURA FAZ: liga o Fe e completa os sinônimos com "fe" e "ferro" (a coluna "Fe" do laudo volta a ser lida); põe 2 casas decimais em B, S, Zn, Cu, Mn e Fe onde não havia valor; e devolve a ordem padrão dos elementos (MO, pH, m%, V%, CTC, P, K…), com as demais variáveis depois, na mesma ordem relativa. O QUE ELA NÃO FAZ: não mexe num catálogo saudável, não religa variável que o usuário desligou, não troca casas decimais já definidas e não desfaz a ordem que o usuário arrumou nas setas — tudo isso fica fora da assinatura. A única correção fora dela é pôr o sinônimo "fe" no Fe quando faltar escrito exatamente assim (só acrescenta; " FE " com espaço ou maiúscula não casa com a coluna do laudo e não conta). Ordem personalizada que existia antes de hoje não tem como voltar: refaça nas setas do Perfil.',
    'O app de campo ATUAL não semeia nem poda catálogos da Biblioteca na nuvem: o boot dele não baixa essas coleções e o envio delas fica travado no aparelho (conferido de novo nesta versão). O aparelho de origem precisa atualizar o app de campo pela loja.',
    'Verificação: teste:catalogo 26/26 (16 novos: 2 casas nos micros da conta nova, assinatura reconhecida, correções de Fe, casas e ordem das complementares, catálogo saudável intocado, escolhas do usuário preservadas, Fe ligado sem "fe" nos sinônimos, " FE " não conta, casas já definidas mantidas, gêmeas, idempotência, conta nova na ordem padrão), teste:janela 11/11, teste:lab 43/43 e teste:ordem 13/13; npx tsc --noEmit limpo e npm run build de produção passou.',
  ],
  ...
  '2.156.0': [
    ...
    'POR QUE ELA NÃO APARECIA: desde a 2.64.0 a lista por zona só existia para cenário aplicado no modo "Por zona de manejo". Quem processou a Fertilidade EM ZONA e aplicou a recomendação no modo padrão (Interpolação) via o mapa chapado por zona — "V 20 m · zona | CTC 20 m · zona" no rodapé — e nenhum número por zona, porque o cenário não trazia a taxa por zona e a tela não tinha de onde tirar. E a lista de antes, mesmo quando aparecia, trazia só a taxa: nem área, nem volume.',
    'DUAS ORIGENS, E A TELA DIZ QUAL É. Cenário POR ZONA (caixa verde, "Recomendação por zona"): a taxa é a exata, calculada direto da equação no laudo da zona, como sempre foi. Cenário INTERPOLADO (caixa azul, "Volumes por zona — prévia lida do mapa"): a taxa de cada zona é a média ponderada dos pixels do mapa da dose cujo centro cai nela, com o mesmo peso de cobertura da tonelagem do cenário; se a dose varia dentro da zona, a faixa mín–máx aparece ao lado, para ninguém tomar a média por taxa fixa. Com a Fertilidade processada por zona o mapa já é chapado e a média é o próprio valor da zona — e o texto da caixa diz isso, e diz que o modo "Por zona de manejo" é o que dá a taxa direto do laudo e o Shapefile com um polígono por zona.',
    'NÃO É O DETECTOR DE "MAPA CHAPADO = POR ZONA" que existiu na 2.63 e foi removido de propósito: nada muda de modo, de arquivo nem de cálculo. É uma leitura do mapa que já está na tela, com a origem escrita no título.',
    'A ÁREA DE CADA ZONA É FATIADA DA ÁREA DO TALHÃO, a mesma régua do painel de Zonas e da Prescrição — as zonas somam o talhão, e o hectare que aparece aqui é o mesmo de lá. Volume = taxa × área, convertido de kg para t quando a dose é em kg/ha (em t/ha a taxa sai com duas casas, para 0,4 não virar "0"). A tonelagem do produto, logo acima, é calculada pixel a pixel sobre o talhão inteiro, então a soma das zonas pode diferir dela em décimos — e quando a diferença passa de 1% a própria tabela escreve o desvio, em vez de deixar dois totais discordando calados.',
    'ZONA SEM NÚMERO SAI MARCADA E FORA DA SOMA, com o motivo: no cenário por zona, uma zona do zoneamento atual que não tem taxa no cenário (faltou laudo, ou o zoneamento mudou depois do cálculo — antes esse casamento parcial passava batido); no interpolado, uma zona sem nenhum pixel do mapa dentro. Na amostragem composta a tabela é por CÉLULA, com o nome que a aba Amostragem usa, e rola dentro da gaveta quando são muitas.',
    'Verificação: teste:volzona (novo, 9 testes — taxa exata × área, kg→t e t/ha, mapa chapado sem variação, mapa interpolado com faixa, NaN fora do talhão, zona sem pixel, prioridade do laudo sobre o mapa); teste:dosezona 13/13 e teste:cobertura 8/8; npx tsc --noEmit limpo em src/ e npm run build de produção passou. Revisão independente pediu e recebeu: pré-filtro por caixa de cada zona antes do ponto-em-polígono (um grid de 280×280 com seis zonas de milhares de vértices levava ~2 s travando a tela a cada troca de dose; agora é instantâneo), o aviso de desvio no total e a zona ausente do cenário explicada.',
  ],
  // [43] Catálogo de variáveis protegido contra apps de campo desatualizados
  '2.155.0': [
    'PENDÊNCIA 43 — AS CASAS DECIMAIS DO MICRONUTRIENTE NÃO "VOLTARAM SOZINHAS": ELAS FORAM APAGADAS NA NUVEM POR UM APP DE CAMPO ANTIGO. O rastro está nos dados e nos logs do Supabase: em 13/09 às 14:41 (Brasília) um celular Android rodando um build do app de campo de antes de 05/08 (portanto sem a correção de 27/08) abriu a Coleta, semeou do zero o catálogo de variáveis (as 64 linhas atuais nasceram nesse instante, criadas pela conta de um operador) e, no primeiro envio, PODOU na nuvem tudo que não estava no seed — o catálogo real, com as casas decimais dos micronutrientes e a ordem dos elementos. É exatamente o defeito corrigido na 2.80.0 (27/08): o app novo não faz mais isso, mas o app velho, instalado num aparelho que nunca atualizou, continua fazendo a cada abertura.',
    'A DEFESA AGORA É NO BANCO, PORQUE É O ÚNICO LUGAR QUE UM APP VELHO NÃO CONTORNA. A plataforma e o app de campo passam a se identificar em toda requisição (cabeçalho x-invicta-versao), e a regra de segurança do banco (docs/seguranca-rls.sql, seção 3b) só aceita gravar, alterar ou apagar nos CATÁLOGOS da Biblioteca — Preferências de Análise, Laboratórios, Perfis de planilha, Perfis, Equações, Recomendações, Insumos, Exportação, Propósitos, Cultivares e os padrões de amostragem/elementos — quando o cliente se identifica com versão igual ou acima da mínima. Quem não se identifica tem a gravação do seed recusada pelo banco, e o envio para aí, antes da poda; e a poda em si, se chegasse a rodar, não apagaria nada, porque para quem não se identifica as linhas ficam invisíveis. A régua fica numa função do banco e sobe por SQL, sem republicar nada. A versão que vale é a da plataforma (2.x), também no app de campo, que é cortado do mesmo código.',
    'ORDEM DE APLICAÇÃO, E ELA IMPORTA: primeiro publicar esta versão, DEPOIS rodar docs/seguranca-rls.sql no SQL Editor. Ao contrário, a plataforma no ar (2.154.0, que ainda não manda o cabeçalho) ficaria sem conseguir salvar na Biblioteca até a publicação. Abas abertas na versão antiga voltam a gravar depois de recarregar — a fila de sincronização guarda a pendência local e reenvia (só EXCLUIR um item de catálogo numa aba antiga parece dar certo e o item volta no próximo boot; recarregar resolve).',
    'O QUE NÃO ENTRA NA RÉGUA, DE PROPÓSITO: Legendas, Grades, Safras e Análises Foliares. O app de campo enviado às lojas hoje (ainda sem o cabeçalho) grava nelas nas migrações e seeds do boot; bloqueá-las deixaria o campo com o selo de sincronização em erro. As coleções protegidas são exatamente as que o app de campo nunca escreve.',
    'O QUE NÃO DÁ PARA DESFAZER: o catálogo apagado em 13/09 não tem histórico no banco. As casas decimais de B, S, Zn, Cu, Mn e Fe já foram regravadas com 2 casas em 14/09; a ordem dos elementos voltou ao padrão de fábrica e, se estava personalizada, precisa ser refeita nas setas do Perfil. E o aparelho de origem precisa atualizar o app de campo pela loja — até lá ele segue tentando e sendo recusado.',
    'Verificação: npx tsc --noEmit limpo em src/; teste:catalogo 10/10 e teste:janela 11/11; a comparação de versão (2.155.0 ≥ 2.9.0 verdadeiro, 2.154.0 ≥ 2.155.0 falso, 2.80.0 ≥ 2.155.0 falso) e a leitura do cabeçalho foram validadas por SELECT no projeto Supabase; revisão independente aprovou a régua contra as demais políticas e o boot do app de campo.',
  ],
  ...
  '2.153.0': [
    ...
    'QUATRO MÉTODOS RODAM LADO A LADO, NÃO UM SÓ. DRIS (com as quatro funções f da literatura — Alvarez-Leite, Jones, Beaufils e a variante robusta), CND (índice IZ e CND-r² sobre a transformação clr — a distância multivariada de Mahalanobis fica REGISTRADA COMO NÃO CALCULADA nesta versão, porque a norma não guarda a matriz de covariância inversa e estimá-la de uma amostra só seria vender gato por lebre), faixa de suficiência (o mínimo–máximo clássico) e chance matemática (a frequência do teor na população de alta produtividade). Os quatro são calculados sobre a MESMA amostra e apresentados juntos, com uma MATRIZ DE CONCORDÂNCIA mostrando onde os métodos concordam e onde divergem — divergência é informação, não seria honesto escondê-la atrás de um número só.',
    'O CONSENSO ENTRE OS QUATRO VIRA CLASSE DE WADT (de Deficiente a Excessivo) e uma ORDEM DE LIMITAÇÃO — qual nutriente corrigir primeiro. E cada diagnóstico sai com um ÍNDICE DE CONFIANÇA: cai quando falta norma, quando a norma é de outro órgão da planta, quando a população de referência é pequena ou o laudo está incompleto — a nota nunca finge uma certeza que os dados não sustentam.',
    'NO TALHÃO, UMA ABA NOVA: "FOLIAR / NUTRIÇÃO". Formulário de lançamento, os quatro métodos lado a lado, HISTÓRICO ENTRE SAFRAS (a evolução de cada nutriente ano a ano) e mapa por zona quando o laudo é georreferenciado — o mesmo padrão de navegação das outras abas do talhão.',
    'NO PAINEL FOLIAR (novo, ao lado dos demais painéis), a VISÃO MULTI-TALHÃO: todos os laudos foliares da conta num só lugar, para comparar entre talhões e safras sem entrar um por um. De lá também se faz a IMPORTAÇÃO de laudo em CSV/XLSX (com a mesma esteira de detecção de unidade e órgão que o laudo de solo já usa) e o GERADOR DE NORMAS PRÓPRIAS — que cruza os laudos foliares importados com os mapas de colheita para construir uma norma de alta produtividade específica do cliente, em vez de depender só da literatura.',
    'A BIBLIOTECA GANHA A CATEGORIA "ANÁLISES FOLIARES", com as normas de fábrica de soja: Kurihara (2013), CONFERIDA contra a fonte primária, e a norma clássica da Embrapa, que entra MARCADA como não conferida na fonte primária até alguém validar cada faixa contra o documento original — errar por excesso de cautela aqui é a opção certa.',
    'RELATÓRIO EM PDF, individual (uma amostra) e em lote (todas as amostras de um mesmo talhão e ano num PDF único, com o histórico entre safras saindo uma vez só no fim), com os quatro métodos, a matriz de concordância e a ordem de limitação — o mesmo que a tela mostra, para o produtor levar impresso.',
    'A REGRA QUE ATRAVESSA O MÓDULO INTEIRO: SEM NORMA, SEM NÚMERO. DRIS e CND só rodam quando existe norma cadastrada para aquela cultura E aquele órgão da planta — sem isso o método fica null com o motivo declarado, nunca um índice inventado. E um laudo incompleto (faltando nutriente que a norma exige) não roda CND: a transformação clr divide cada teor pela média geométrica de TODOS os componentes, então tirar um deles desloca o vetor inteiro em bloco — misturar amostra incompleta com norma completa daria um número plausível e sem significado.',
    'Verificação: teste:foliar 55/55, teste:foliar-normas 30/30, teste:foliar-fabrica 9/9, teste:foliar-graficos 9/9, teste:foliar-importacao 22/22, teste:lab 43/43, teste:portal 30/30, npx tsc --noEmit limpo e npm run build de produção passou.',
  ],
  // [41] Amostragem composta: células quadráticas com as subamostras dentro
  '2.152.0': [
    'PENDÊNCIA 41 — A AMOSTRAGEM COMPOSTA VIROU UMA FERRAMENTA. Talhão → Amostragem ganhou uma quarta opção, ao lado de Grid, Zona de Manejo e Registros de campo. Ela existe para o talhão pequeno de pouca variabilidade — a sobra que o pivô não irriga, o talhão que o produtor só quer amostrar por obrigação —, onde uma coleta composta resolve e, até aqui, a informação simplesmente não ficava guardada em lugar nenhum.',
    'COMO SE USA, EM DUAS DECISÕES. Primeiro você diz QUANTAS AMOSTRAS VÃO AO LABORATÓRIO — um número fixo (1, 2, 4…) ou hectares por amostra, e a tela mostra a conta ("18,4 ha ÷ 5 = 4 amostras"). O talhão é então recortado em CÉLULAS QUADRÁTICAS de área equivalente, bem distribuídas e alinhadas ao eixo maior da área. Depois você diz QUANTAS SUBAMOSTRAS cada célula leva (os furos que o operador dá e mistura num saco só, padrão 10) — e elas são distribuídas dentro de cada célula.',
    'EXATAMENTE O NÚMERO QUE VOCÊ PEDIU, COBRINDO O TALHÃO INTEIRO. Um reticulado cru sobre um contorno irregular nunca dá a conta certa: sobram tiras finas na borda e o número de pedaços varia em degraus. Aqui as sobras são fundidas na célula vizinha com a maior divisa compartilhada, e a maior célula é partida ao meio quando falta uma — sempre sem deixar buraco e sem sobrepor. É essa garantia que faz a soma dos volumes fechar depois: cada hectare pertence a uma amostra, e a uma só.',
    'O DESENHO DAS CÉLULAS NÃO SORTEIA NADA. Ele depende só da geometria, do número de amostras e do ângulo — gerar de novo dá exatamente o mesmo mapa. O operador sai a campo com um papel impresso semanas antes, e ele tem de continuar valendo. A aleatoriedade e o botão "Refazer" mexem só nos FUROS.',
    'NO APP DE CAMPO É A EXPERIÊNCIA DA ZONA DE MANEJO. O operador vê as células desenhadas e coloridas, com os furos indicados dentro de cada uma, e cada ponto rotulado célula-sequencial ("2-3") — o mesmo padrão que ele já usa. A divisa desce junto com a grade (fica gravada nela, não no talhão), então duas amostragens do mesmo talhão podem ter recortes diferentes sem uma atropelar a outra.',
    'UM SACO POR CÉLULA, NÃO UM POR FURO. Etiquetas, carta de conferência para o laboratório, KML e Shapefile saem pela MESMA conta da grade de zonas composta: quatro células com dez furos cada dão 4 etiquetas, não 40. O KML/SHP leva as células como polígonos ("Celula 03"), que é o que diz onde um saco termina e o outro começa.',
    'DO LAUDO ATÉ O VOLUME, SEM INTERPOLAR. Quando o laudo é de uma composta, a Fertilidade abre direto no mapa por área (o seletor de modo nem aparece): cada célula é pintada com o resultado dela, chapado. Interpolar valores compostos cravados em coordenadas de subamostra não significaria nada — e com uma amostra só seria impossível. A Recomendação segue igual: a equação roda uma vez por célula, e o volume de cada uma sai da sua área.',
    'O VÍNCULO CÉLULA ↔ AMOSTRA É EXATO, não adivinhado. Nas zonas de manejo ele é deduzido pela localização do ponto que cai dentro da zona, o que tem dois jeitos conhecidos de errar (furo em cima da divisa; laudo sem uma das áreas deslocando todas as seguintes). Na composta esse par é conhecido por construção e é ele que a tela, o relatório e a prescrição usam — a classe inteira de erro deixa de existir.',
    'O RELATÓRIO (BOOK) TAMBÉM SABE. Ele passou a montar as áreas da composta a partir da grade, como a tela. Sem isso ele cairia nas zonas de manejo do talhão — ou em nenhuma — e escreveria os valores no lugar errado, que foi exatamente o defeito da pendência 38.',
    'DESMEMBRAR E FUNDIR TALHÃO NÃO CORROMPEM MAIS NADA. Uma composta nunca é fundida com outra grade (o número do ponto ali é o número da CÉLULA, e renumerar arrebentaria o par com o saco): ela migra inteira. E ao desmembrar, as células — recortadas do talhão inteiro — são descartadas com um aviso, em vez de ficarem cobrindo terra que não é mais daquele talhão.',
    'A REGRA DA EDIÇÃO MANUAL DOS PONTOS AGORA É UMA SÓ. Mover, adicionar e remover furos funciona como nas zonas (o ponto fica preso na sua célula, mover preserva o número e protege as coletas já feitas, add/remover fecham o sequencial sem buraco) — e as duas abas passaram a compartilhar o mesmo código, para a próxima correção não entrar só numa delas.',
    'Verificação: teste:gradecomposta 27/27 (novo — entre eles o que sorteia 200 pontos dentro do talhão e exige que cada um caia em exatamente UMA célula, que é o que pega buraco e sobreposição do recorte), teste:gradezonas 24/24, teste:fertzona 16/16, teste:grids 33/33, teste:porpoligono 15/15, teste:dosezona 13/13, teste:elo 12/12, teste:relatorio-grade, teste:zonas 12/12, teste:rotulos-mapa 22/22, teste:desmembrar 8/8, teste:fundir 8/8. npx tsc --noEmit limpo e npm run build ok.',
  ],
  ...
  '2.147.0': [
    ...
    'POR QUE ISSO IMPORTA: até aqui, imagem só era guardada se alguém abrisse a aba e pedisse. Talhão que ficou duas semanas sem visita ficava com duas semanas de buraco na série — e passagem de satélite não volta atrás.',
    'ELE SÓ GUARDA O QUE PRESTA, por quatro regras que você controla em cada talhão: quanto do TALHÃO precisa estar limpo de nuvem e sombra (padrão 70%), o vigor médio mínimo — que barra solo exposto e pós-colheita (padrão 0,15), a nuvem máxima da cena (padrão 20%, um filtro barato antes de baixar qualquer banda) e o intervalo mínimo entre duas imagens guardadas (padrão 5 dias, o freio do tamanho do banco).',
    'AS REGRAS SÃO AS MESMAS DA TELA. Os limiares que o gráfico de seleção usa para pintar um ponto de verde são exatamente os que o robô aplica de madrugada — conferido caso a caso por teste automático nos dois lados. Se a tela diz que a cena presta, o robô não a recusa.',
    'ONDE MARCAR: dentro do talhão (aba NDVI, cartão "Busca automática de madrugada") para um; ou em Produtores → fazenda → Talhões → "Monitoramento por satélite", que lista a fazenda inteira com caixas de seleção, "todos / nenhum / só os não monitorados", e aplica as regras ao lote de uma vez. Talhão sem limite aparece bloqueado, com o motivo — marcar seria prometer o que não dá para cumprir.',
    'VOCÊ VÊ O QUE ELE FEZ, E POR QUE NÃO FEZ. Cada execução vira um registro com data, quantas cenas avaliou, quantas salvou e — cena por cena — o motivo da recusa ("talhão encoberto", "vigor abaixo do mínimo", "muito perto da anterior"). É o que permite ajustar o limiar certo em vez de chutar.',
    'E DÁ PARA APAGAR TUDO DEPOIS. Na aba "Camadas salvas", cada camada agora tem caixa de seleção, um filtro "Automáticas / Feitas à mão" e o botão de excluir as selecionadas — que apaga DO BANCO, não só da tela. As geradas pelo robô levam um 🤖 e mostram quanto do talhão estava limpo. Ao desligar o monitoramento, a plataforma pergunta se você quer apagar as automáticas daquele talhão; manter é o padrão, porque são meses de processamento e a pergunta não pode ser respondida sozinha.',
    'ANTES ERA IMPOSSÍVEL APAGAR EM PARTE: a exclusão só existia por prefixo, e automáticas e manuais dividem o mesmo prefixo — apagar as do robô levaria junto as suas. Agora a exclusão é por camada escolhida, e limpa também o cache do aparelho (senão o arquivo apagado ficaria ocupando disco para sempre, invisível).',
    'DETALHE DE ENGENHARIA QUE VOCÊ NUNCA VAI VER, MAS QUE SEGURA A NOITE: o servidor roda em dois processos, e uma trava no banco garante que só um deles execute a varredura. Cada imagem é gravada assim que fica pronta, então se o processo for reciclado no meio (ele é, de propósito, para não inchar a memória), o substituto retoma de onde parou em poucos minutos — sem refazer nada e sem duplicar.',
    'CBERS-4A FICA DE FORA DO AUTOMÁTICO: o sensor não tem banda de qualidade, então não há como afirmar que o talhão está sem nuvem. Ele continua disponível para escolha manual, com o aviso.',
    'ATIVAÇÃO: o robô só arma se o servidor tiver MSR_AGENDA=1 (com as credenciais do banco). Sem isso, tudo continua exatamente como antes e a tela de "/health" diz o motivo — nada falha em silêncio.',
    'Verificação: teste:msr-regras 35/35 em Python (as 4 regras isoladas, os limiares exatos de borda, o formato de data da trava, o gzip do raster que o navegador precisa ler e a leitura do limite do talhão nos 4 formatos que o banco guarda), teste:msr 35/35 e teste:grafcenas 24/24 no front. npx tsc --noEmit limpo, npm run build ok, ESLint sem aviso novo.',
  ],
  // [40] Gráfico das cenas do período e processamento em lote
  '2.146.0': [
    'PENDÊNCIA 40 — AGORA DÁ PARA PEDIR TRÊS ANOS DE IMAGENS E ESCOLHER AS BOAS NUM GRÁFICO. A aba NDVI/Satélite ganhou atalhos de período (3 meses · 1 ano · 2 anos · 3 anos) e uma visão de GRÁFICO do período inteiro, com arrasto para marcar um intervalo, avaliação das cenas desse intervalo e processamento EM LOTE das escolhidas — de uma vez, já salvas.',
    'ANTES O HISTÓRICO LONGO NEM CHEGAVA NA TELA: o servidor devolvia no máximo 30 cenas, sempre as mais recentes, e o resto sumia sem aviso. No pivô de teste, três anos de Sentinel-2 são 335 cenas — o que aparecia era o último trimestre e nada mais. O teto foi para 800.',
    'E A NUVEM QUE VOCÊ VIA NÃO ERA A DO SEU TALHÃO. O "☁ 5%" do cartão é a nuvem da CENA INTEIRA, um quadrado de ~110 km; o talhão é um ponto dentro dele. Medido neste mesmo pivô: a cena de 17,6% de nuvem deixou 69% do TALHÃO limpo, e a de 9,2% deixou só 58% — a menos nublada era a pior das duas. Quem escolhia pelo número do cartão escolhia errado, e só descobria depois de processar.',
    'O GRÁFICO RESPONDE A PERGUNTA CERTA, EM DUAS ETAPAS. Ele abre na hora com as barras da nuvem da cena (esse dado vem de graça do catálogo). Você arrasta o intervalo que interessa — uma safra, um mês — e clica em "Avaliar": o servidor lê a máscara de nuvem RECORTADA NO TALHÃO e devolve quanto dele está realmente limpo, mais o vigor médio (NDVI). Aí as barras viram pontos verdes (aceita) ou laranja (reprovada) na altura do talhão limpo, e uma linha roxa desenha a evolução do vigor.',
    'A AVALIAÇÃO É BARATA DE PROPÓSITO: ela lê numa grade grossa, com o pixel escolhido pelo tamanho do talhão (20 m nos pequenos, até 60 m nos grandes), e para de ler assim que o talhão já reprovou no percentual — o que a deixa ~7x mais rápida nas cenas nubladas, que são a maioria. Conferido contra o processamento fino de 10 m na mesma cena: 100% limpo dos dois lados e NDVI médio 0,163 nos dois. Barato não é impreciso.',
    'A LINHA DO VIGOR É CORTADA NOS VAZIOS: quando passam mais de 30 dias sem nenhuma imagem aproveitável, a linha simplesmente para e recomeça depois. Uma reta ligando março a maio seria um dado que ninguém mediu.',
    '"MARCAR AS MELHORES" faz a escolha por você, e a regra cabe numa frase: divide o período em faixas de N dias (você escolhe o N) e, em cada faixa, fica com a cena que tem MAIS talhão limpo — empatou, a de maior vigor. Depois é só "Processar e salvar": ele roda uma por uma, mostra o progresso e guarda cada uma como camada oficial, sem pedir "Manter" vinte vezes.',
    'CBERS-4A APARECE MAS NÃO É ESCOLHIDO SOZINHO: o sensor não tem banda de qualidade, então não há como afirmar que o talhão está sem nuvem. Ele entra no gráfico para você olhar; a marcação automática só o considera se você tiver escolhido CBERS como fonte de propósito.',
    'E AS MINIATURAS GANHARAM TETO. Cada prévia RGB é uma leitura de imagem no servidor; sem limite, pedir três anos dispararia mais de 300 de uma vez e derrubaria o processamento de todo mundo. Agora carregam as 24 primeiras nos cartões, e nenhuma enquanto você estiver no gráfico — que é onde se olha período longo.',
    'Verificação: teste:msr 35/35 (as 4 regras isoladas, os limiares exatos de borda, a ordem das checagens e a estabilidade de "marcar as melhores" sob qualquer ordem de entrada) e teste:grafcenas 24/24 (o eixo é tempo e não posição na lista, ida e volta do arrasto, quebra da linha nos vazios). Smoke contra o catálogo real: 30 → 335 cenas em 3 anos, 1,5 s por cena avaliada (0,21 s nas cortadas). npx tsc --noEmit limpo, npm run build ok e ESLint sem nenhum aviso novo.',
  ],
  ...
  '2.142.0': [
    ...
    'ERAM TRÊS PULOS POR CLIQUE, e todos foram embora: (1) o bloco de resultado nascia inteiro no topo; (2) antes disso ele era DERRUBADO e remontado, porque abrir um cenário passava pelo mesmo estado de "carregando" do botão Aplicar; (3) abrir um cenário por zona estando em "Interpolação" trocava o modo do mapa e o texto de ajuda ia de 1 para 3 linhas, empurrando tudo mais uma vez. Agora abrir um cenário só mexe no que está DENTRO da gaveta dele: o "Abrindo os mapas…" e qualquer erro aparecem ali, ao lado do cartão, e não numa faixa no alto da tela.',
    'A LISTA DE PRODUTOS PASSOU A SER UMA SÓ. Ela aparecia duas vezes — no bloco de cima e na gaveta — com a ★ funcionando só na de cima. Ficou a da gaveta, com a ★ acionável quando o cenário está aberto: clique no produto para ver o mapa, clique na ★ para marcar o que vai ser utilizado (é o que gera arquivo). Um olho verde marca o produto que está no mapa. A lista rola dentro dela mesma (~10 linhas em vez de 6), então financeiro, legenda e "Explicar com IA" continuam a uma rolagem curta mesmo num cenário de 29 produtos.',
    'A ★ TAMBÉM PAROU DE FAZER O CARTÃO PULAR PARA O TOPO DA LISTA. Marcar um produto regrava o cenário, e regravar atualizava a data de geração — que é o critério de ordenação. Era invisível enquanto a ★ morava no bloco de cima; com ela dentro da gaveta, o cartão saltaria debaixo do seu cursor a cada clique. Agora a marcação é aplicada na lista sem reordenar, e a gravação na nuvem (que é o que as abas Arquivos e Relatórios leem) continua igual.',
    'O FORMULÁRIO DE CÁLCULO VIROU UMA SEÇÃO RECOLHÍVEL, "Nova recomendação". Ele tem quase uma tela de altura e a lista de cenários vive depois dele — recolhido, a lista fica à mão. Em talhão que já tem cenários ele abre recolhido; em talhão sem nenhum, abre aberto (é o único conteúdo da tela). A sua escolha é lembrada por talhão. Depois de "Aplicar e salvar" ele recolhe sozinho e a gaveta do cenário novo abre já com o resultado — o cartão novo é o primeiro da lista, então o resultado nasce à vista sem a tela rolar sozinha.',
    'E O RESULTADO NUNCA MAIS FICA SEM LUGAR PARA APARECER: se o salvamento na nuvem falhar, o cenário calculado entra na lista mesmo assim, com um selo âmbar "não salvo" e o resultado completo dentro. Antes, como o resultado morava no topo, ninguém notava a falha; agora que ele mora no cartão, um cenário sem cartão sumiria da tela.',
    'NA TELA DO PRODUTOR (Recomendações só para ver), uma falha ao abrir os mapas de um cenário era 100% silenciosa — aquele ramo da tela não mostrava erro nenhum. Passa a mostrar, dentro do cartão que falhou.',
    'Verificação: npx tsc --noEmit e npm run build limpos, eslint com os mesmos erros pré-existentes do arquivo (nenhum novo), teste:dosezona 13/13, teste:zona-e2e 24/24, teste:fertzona 10/10 — o cálculo da dose não foi tocado, só de onde o clique vem e onde o resultado é desenhado.',
  ],
  // [S/N] O app parou de pedir login toda hora (a sessão saiu de um armazenamento cheio)
  '2.141.0': [
    'O APP PARAVA DE LEMBRAR QUEM VOCÊ É E PEDIA LOGIN E SENHA A CADA ABERTURA. Não era o login "caindo", nem sessão vencendo: o armazenamento local deste navegador estava CHEIO, e a sessão não tinha onde ser guardada.',
    'COMO ISSO ACONTECIA, EM SILÊNCIO: a biblioteca de login, quando não recebe ordem em contrário, faz um teste de escrita minúsculo no armazenamento do navegador ao abrir o app. Se o teste falha — e com o armazenamento cheio ele falha —, ela passa a guardar a sessão SÓ NA MEMÓRIA da aba, sem erro, sem aviso, sem nada no console. Aí toda recarga, toda aba nova, todo F5 voltava para a tela de login. E mesmo quando o teste passava por pouco, o token é renovado de hora em hora e precisa ser REGRAVADO: se essa regravação não coubesse, ficava guardado o token velho, que na volta já não valia — e o app pedia login de novo.',
    'A SESSÃO NÃO DISPUTA MAIS ESPAÇO COM O CACHE. Agora o app diz explicitamente onde guardar o login: primeiro o armazenamento normal; se ele recusar, o IndexedDB, que tem cota na casa dos GIGAbytes e nunca enche com uma sessão de poucos KB. Só se os dois falharem (navegação privada com tudo bloqueado) é que a sessão fica na memória — e aí o selo de armazenamento cheio acende no alto da tela, em vez de o app simplesmente esquecer você.',
    'E O ARMAZENAMENTO PAROU DE ENCHER. Os laudos importados (`inv_lab`) e as grades de amostragem com todos os pontos (`inv_grades`) eram as duas últimas coleções por talhão que ainda ficavam no armazenamento pequeno do navegador — um laudo de 84 amostras por ~15 variáveis e uma grade de 84 pontos, POR talhão e POR ano, somam megabytes rápido. Passam para o IndexedDB, junto com os polígonos, a condutividade e as outras. A mudança é automática: na primeira abertura desta versão os valores são movidos e o espaço é DEVOLVIDO — nada a fazer, nada a reimportar, os dados continuam os mesmos.',
    'ISSO TAMBÉM FECHA A CAUSA DO "FALHA AO REABRIR: THE QUOTA HAS BEEN EXCEEDED." da versão anterior: lá o sintoma foi contido (gravar uma preferência nunca mais derruba a ação); aqui a causa foi removida.',
    'SE O SELO DE ARMAZENAMENTO CHEIO AINDA APARECER depois desta versão, aí sim vale limpar dados de sites no navegador — mas é para ser raro. Seus dados estão na nuvem e voltam sozinhos; o que enche é o cache local.',
    'Testes: teste:sessao (novo, 11 asserções) trava o contrato do armazenamento da sessão — armazenamento cheio manda a sessão para o IndexedDB, ela sobrevive à recarga, logout limpa os três lugares, e um armazenamento que lança até na LEITURA não derruba o app.',
  ],
  // [S/N] Clicar na recomendação abre o mapa dela na hora (adeus, ícone de pasta)
  '2.140.0': [
    'CLICAR NUMA RECOMENDAÇÃO PÕE O MAPA DELA NA TELA — em qualquer cenário salvo, em qualquer modo. Antes só a lista do cenário recém-calculado fazia isso: para ver o mapa de um cenário já salvo era preciso achar o ÍCONE DE PASTA ao lado do nome, clicar, e ainda responder a um "reabrir substitui os N mapas que estão na tela. Continuar?". Três passos e uma pergunta para o gesto mais óbvio da tela. O ícone de pasta SUMIU.',
    'AGORA SÃO DOIS CLIQUES NO MÁXIMO: clicar no cenário abre a gaveta com os produtos E já joga o primeiro mapa no mapa ao lado; clicar em qualquer recomendação de dentro troca para o mapa dela. Um olho verde marca a que está na tela, e uma rodinha aparece enquanto os mapas do cenário são baixados. Fechar a gaveta é só clicar de novo — não recarrega nada.',
    'VALE IGUAL PARA DOSE INTERPOLADA EM GRADE E PARA TAXA POR ZONA. Quem decide é o próprio cenário: se as doses foram feitas por zona, o seletor "Por zona" se ajusta sozinho ao abrir. Esse ajuste automático tinha um efeito colateral escondido: abrir um cenário POR ZONA estando em "Interpolação" carregava os mapas e os APAGAVA no mesmo instante — trocar o modo é também o gesto que limpa a tela, e o app não distinguia os dois. Dava tela vazia, sem erro nenhum. Corrigido.',
    'CORRIGIDO — "FALHA AO REABRIR: THE QUOTA HAS BEEN EXCEEDED." O erro vermelho não tinha nada a ver com o cenário, que na verdade TINHA sido carregado. O app guarda no navegador qual modo de mapa você usa em cada talhão — dez bytes de conveniência — e essa gravação era crua: com o armazenamento local cheio ela estourava e derrubava a abertura inteira, deixando a tela pela metade e um erro apontando para o lugar errado. Agora a preferência é gravada por um caminho que NUNCA derruba a ação: se não couber, o modo só não é lembrado na próxima visita, e quem avisa que o armazenamento está cheio é o selo de sincronização no alto da tela, que é o lugar dele.',
    'SE ESSE SELO DE ARMAZENAMENTO CHEIO APARECER, vale limpar dados de sites no navegador: o que enche é o cache local deste navegador — seus dados estão na nuvem e voltam sozinhos.',
    'A tela do produtor (Recomendações só para ver) passou pelo mesmo caminho: clicar no cenário e clicar no produto usam agora exatamente a mesma função de quem calcula, em vez de uma versão paralela que perdia o produto escolhido quando o cenário trocava o modo do mapa.',
    'Verificação: npx tsc --noEmit e npm run build limpos. Entrega de tela (cliques e estado do componente) — não há módulo puro novo para cobrir com os testes de Node; os existentes seguem passando.',
  ],
  ...
  '2.136.0': [
    ...
    'DOIS DEFEITOS IMPEDIAM GERAR QUALQUER PACOTE PARA A LOJA. O primeiro: o arquivo `src/lib/firebase.ts` reapareceu na pasta sem ter sido commitado — resto da migração para o Supabase, feita na 1.72.0 — e derrubava o build do app com "Cannot find module firebase/app". Ninguém importava esse arquivo; ele foi removido. O segundo: a numeração de versão do Android estourou. O número interno saía de `maior × 10.000 + menor × 100 + correção`, fórmula que só comporta "menor" até 99 — e a plataforma passou da 2.99 faz tempo. O empacotamento recusava rodar. A fórmula agora é `maior × 1.000.000 + menor × 1.000 + correção` (2.136.0 = 2.136.000), continua sempre crescente e gera números MAIORES que os antigos, então a loja segue aceitando as atualizações. O app de iPhone tinha o mesmo defeito e foi corrigido junto.',
    'O GRADLE NÃO COMPILA DE DENTRO DO ONEDRIVE, e agora o empacotamento contorna isso sozinho. O empacotador do Android copia arquivos preservando os atributos, e para qualquer arquivo cuja origem esteja em `~/Library/CloudStorage/` o sistema recusa a operação — o build morria com "Operation not permitted" sem dizer o motivo. Não há ajuste do Gradle que resolva, e mudar a pasta de saída não adianta: o problema é o lado de origem. O `npm run android:release` passou a espelhar o projeto para uma pasta local fora da nuvem, compilar lá e trazer o pacote de volta. ATENÇÃO: compilar pelo botão do Android Studio com o projeto aberto na pasta do OneDrive continua falhando — use o comando.',
    'PUBLICAR FICOU UM COMANDO SÓ. O `npm run android:release` faz tudo — build, conferências, compilação e entrega — e o pacote sai em `loja/INVICTA-Coleta-<versão>.aab`, pronto para subir. As conferências agora também apontam o SDK do Android sozinhas (faltava o `local.properties`, que não é versionado) e RECUSAM gerar o pacote sem a chave de assinatura, em vez de entregar um arquivo que parece pronto e só é recusado lá na loja.',
    'A CHAVE DE ASSINATURA GANHOU UM ASSISTENTE: `bash scripts/criar-chave-android.sh`. Ele pede a senha sem mostrá-la na tela e sem deixá-la no histórico do terminal, cria a chave, grava a configuração do build e mostra a impressão digital para você guardar junto do backup. Vale lembrar que perder essa chave ou a senha significa não poder mais atualizar o app publicado.',
    'Os textos da ficha da Play Store — descrições, notas de versão, tabela de segurança dos dados, classificação — estão prontos em `loja/ficha-play-store.md`, junto do ícone 512×512 e da imagem de destaque. O passo a passo da publicação está em `docs/publicar-android.md`.',
  ],
  // [S/N] Quem entra por link de convite não passa mais por aprovação
  '2.135.0': [
    'PENDÊNCIA S/N — QUEM ENTRA POR LINK DE CONVITE NÃO PEDE MAIS APROVAÇÃO. O link já sai daqui com categoria, papel, perfil de permissões e vínculos escolhidos por você; pedir "aprovar como?" depois era a MESMA decisão duas vezes. Agora a pessoa abre o link, cria a senha e entra ativa, com exatamente o acesso que o convite dizia. Vale para o convite individual e para o link de grupo.',
    'POR QUE ISSO JÁ ERA PARA VALER DESDE A 2.116 E NÃO VALIA: a tela do convite só reconhecia o convite se a lista `inv_convites` estivesse no NAVEGADOR de quem se cadastrava — e ela só está no navegador de quem administra. No celular de quem recebeu o link o convite era sempre "desconhecido", e o cadastro caía na fila. Pior: mesmo quando era reconhecido, o servidor recusava gravar "ativo" (a regra de segurança do banco só aceitava "aguardando aprovação"), e o app caía na fila de novo. Eram duas travas em série; as duas foram removidas.',
    'A LIBERAÇÃO PASSOU A SER DO SERVIDOR, não do navegador. Uma função no banco lê o convite pelo código do link, confere validade e e-mail, e MONTA o cadastro a partir do convite — papel, categoria, perfil e vínculos não passam pelo navegador de quem se cadastra. Sem isso, aceitar "ativo" vindo da tela deixaria qualquer convidado se promover a agrônomo pelo console do navegador. Continua recusando link cancelado, vencido, já usado (no individual) e e-mail diferente do convidado; papel de owner/admin nunca sai de um link.',
    'A FILA QUE JÁ EXISTE SE RESOLVE SOZINHA: ao abrir a Central de Acessos, todo cadastro parado em "aguardando aprovação" que veio de um convite conhecido é liberado na hora, com o papel e o acesso do próprio link — e a tela avisa quantos foram. Quem estava preso há dias aparece já ativo na aba da categoria dele. Só continua esperando decisão sua quem veio de um convite que este aparelho não conhece ou de um link que você cancelou.',
    'DE QUEBRA, O PERFIL DE PERMISSÕES DO LINK PAROU DE SE PERDER na aprovação manual: os campos que guardavam o que o link propunha existiam no registro e nunca eram preenchidos, então a tela de aprovação abria em "leitor/interno" mesmo quando o convite dizia "produtor". Agora ela abre com o que o convite definiu.',
    'ATENÇÃO — PARA A LIBERAÇÃO INSTANTÂNEA VALER, é preciso rodar uma vez o `docs/seguranca-rls.sql` atualizado no SQL Editor do Supabase (é ele que cria a função de aceite). Sem isso o app continua funcionando: o cadastro cai na fila e a Central de Acessos libera sozinha na primeira vez que você abrir a tela.',
    'Testes: teste:autocadastro passou de 9 para 19 asserções (o aceite pelo servidor, o link de grupo que não se esgota, o convite cancelado recusado, a fila antiga sendo varrida e a prova de que o navegador CONTINUA sem conseguir gravar "ativo" sozinho); teste:convite-regras, de 12 para 17.',
  ],
  // [S/N] Interpolador local no Windows — agora corrigido de verdade (a 2.129.0 não chegou ao código)
  '2.134.0': [
    'O INTERPOLADOR LOCAL VOLTA A ABRIR — no Windows e no Mac. A 2.129.0 descreveu esta correção, mas ela NÃO chegou ao código: o empacotador e o start.ps1 continuaram exatamente como estavam, e quem baixou o pacote depois daquela versão continuou sem conseguir subir o interpolador. Agora está feito e conferido — este aviso fica aqui para que o histórico não diga que foi resolvido antes da hora.',
    'A CAUSA: o pacote de download era montado por uma LISTA MANUAL de arquivos do backend, e o `divisas.py` (criado na 2.107.0, quando o app passou a incorporar divisas internas ao talhão) nunca entrou nessa lista. O `app.py` de dentro do ZIP importa `divisas` na abertura — sem o arquivo, o backend morria no primeiro segundo com "No module named divisas". Quem já tinha o interpolador antigo instalado seguia usando; quem baixou de novo para atualizar ficou sem. Agora TODO .py do backend entra no pacote sozinho, sem lista para esquecer, e o empacotador ABORTA a publicação se algum import dos arquivos empacotados não tiver origem (nem no pacote, nem na biblioteca padrão do Python, nem no requirements.txt) — em vez de publicar um ZIP que só quebra na máquina do usuário.',
    'NO WINDOWS A JANELA FECHAVA SEM MOSTRAR NADA, e o erro acima virava "simplesmente não abre". Quatro causas, as quatro corrigidas no start.ps1: (1) o `pip install` só rodava JUNTO COM A CRIAÇÃO do ambiente — quem já tinha o ambiente atualizava os .py e continuava com as bibliotecas velhas; agora as libs são reinstaladas sempre que o requirements.txt muda, mais um reparo automático se algum import essencial falhar (é o que o Mac já fazia). (2) Nenhuma falha segurava a janela aberta: agora toda falha imprime o motivo em português e espera Enter. (3) O script testa se o backend CARREGA antes de subir o servidor e, se faltar arquivo, diz qual é e manda baixar o pacote de novo, em vez de piscar um traceback. (4) O script abortava sozinho em erro que não era erro: rodava com "parar em qualquer erro" ligado, e o Windows PowerShell trata QUALQUER linha que um programa externo escreva em stderr como exceção — o pip escreve aviso de rotina ali. Agora o que decide é o código de saída de cada comando.',
    'O start.bat também ficou à prova de janela em branco: roda o PowerShell sem o perfil do usuário (um perfil com erro derrubava tudo antes de começar), avisa se o start.ps1 não está na pasta e, se o Windows bloquear a execução do script por política, mostra o comando exato para liberar. E o próprio start.ps1 reconhece quando você deu 2 cliques de DENTRO do .zip — o caso em que o Windows roda de uma cópia temporária e nada funciona.',
    'CONFIGURAÇÕES → "Processamento de mapas": o botão de download agora reconhece o seu sistema e mostra só as instruções dele — "Baixar interpolador local (Windows)" com o caminho do start.bat, ou (Mac) com o caminho do Terminal —, com um link para ver as do outro sistema. O pacote é o mesmo para os dois. O arquivo passa a se chamar `interpolador-local.zip` (o nome antigo continua respondendo, para links já salvos).',
    'PARA VOLTAR A USAR: baixe o interpolador de novo em Configurações e descompacte por cima da pasta antiga (no Windows, 2 cliques em start.bat). O ambiente Python já instalado é reaproveitado; só as bibliotecas que mudaram são baixadas.',
  ],
  ...
  '2.130.0': [
    ...
    'OS DOIS BOTÕES FICAM JUNTOS em Talhão → Limite do Talhão: "Talhão em N áreas — separar uma delas" (só aparece quando há mais de uma área) e "Fundir com outro talhão" logo abaixo. O botão de fundir saiu do topo da ficha — é o mesmo trabalho de acertar o cadastro com o que existe na fazenda, e agora mora no mesmo lugar. Dentro da janela, a aba de separar fica travada, com o motivo, quando o talhão é uma área só.',
    'Renomear um talhão para um nome que já existe continua oferecendo a fusão — só que abre esta janela, já na aba certa e com o outro talhão escolhido. O histórico de fusões segue na ficha do talhão.',
    'Nenhuma regra mudou: a separação continua sem renumerar amostra, a fusão continua pedindo o "estou ciente" e mostrando o plano antes de aplicar, e as duas continuam arquivando o limite anterior como versão do talhão.',
  ],
  // [S/N] Interpolador local não subia mais (faltava um arquivo no pacote de download)
  '2.129.0': [
    'O INTERPOLADOR LOCAL NÃO ABRIA MAIS — nem no Windows nem no Mac — desde a 2.107.0. O pacote de download era montado por uma LISTA MANUAL de arquivos do backend, e o `divisas.py` (criado naquela versão, quando o app passou a incorporar divisas internas ao talhão) nunca foi acrescentado à lista. O `app.py` de dentro do ZIP importa `divisas` logo na abertura: sem o arquivo, o backend morria no primeiro segundo com "No module named divisas". Quem já tinha o interpolador antigo instalado seguia usando; quem baixou de novo para atualizar ficou sem — a atualização é que quebrava. Agora TODO .py do backend entra no pacote sozinho, sem lista para esquecer, e o empacotador ABORTA o deploy se algum arquivo importado ficar de fora (em vez de publicar um ZIP que só quebra na máquina do usuário).',
    'NO WINDOWS A JANELA FECHAVA SEM MOSTRAR NADA, e o erro acima virava "simplesmente não abre". Três causas, as três corrigidas no start.ps1: (1) o `pip install` só rodava JUNTO COM A CRIAÇÃO do ambiente — quem já tinha o ambiente atualizava os .py e continuava com as bibliotecas velhas, então biblioteca nova faltando derrubava o backend e a atualização "não pegava"; agora as libs são reinstaladas sempre que o requirements.txt muda, mais um reparo automático se algum import essencial falhar (é o que o Mac já fazia). (2) Nenhuma falha segurava a janela aberta: agora toda falha imprime o motivo e espera Enter. (3) O script testa se o backend carrega ANTES de subir o servidor e, se não carregar, diz em português para baixar o pacote de novo, em vez de piscar um traceback.',
    'AINDA NO WINDOWS: o script abortava sozinho em erro que não era erro. Ele rodava com "parar em qualquer erro" ligado, e o Windows PowerShell trata QUALQUER linha que um programa externo escreva em stderr como exceção — o pip escreve aviso de rotina ali (aviso de versão nova, por exemplo). Bastava um desses para a janela fechar no meio da instalação. Agora o resultado de cada comando é conferido pelo código de saída, que é o que de fato diz se falhou.',
    'PARA VOLTAR A USAR: baixe o interpolador de novo em Configurações e descompacte por cima da pasta antiga (no Windows, 2 cliques em start.bat). O ambiente Python já instalado é reaproveitado; só as bibliotecas que mudaram são baixadas.',
  ],
  // [S/N] CTC efetiva parando de virar CTC pH 7,0 (importação, zonas de manejo, legenda e PDF)
  '2.128.0': [
    'EXCLUIR UM LAUDO PASSA A APAGAR TAMBÉM OS MAPAS DELE. A linha do laudo sempre saiu de verdade (do aparelho e da nuvem), mas os rasters gerados a partir dela — os mapas da Fertilidade e a gaveta de 20 m que alimenta a dose — ficavam para trás, porque vivem numa coleção própria, gravada sob demanda e fora do sync de listas. Na base havia 1.337 mapas órfãos de 92 importações já excluídas (41% da coleção). Na prática, "excluí a planilha porque importei errado" não desfazia o processamento errado. Agora a exclusão leva os dois — pela tela, pelo desmembramento e pela fusão de talhões.',
    'ZONAS DE MANEJO LIAM O LAUDO POR UMA REGRA DIFERENTE DA INTERPOLAÇÃO — corrigido. Ao preencher a zona, o app pegava a PRIMEIRA linha daquele número de amostra e daquela profundidade, tivesse ou não o nutriente pedido; a interpolação sempre procurou a linha QUE TEM o nutriente. Num laudo em que macro e micro chegam em linhas separadas do mesmo ponto (protocolos diferentes, que é o que impede a fusão), a zona caía na linha sem Ca/Mg/K — justamente onde a CTC efetiva é apagada por não ter de onde ser calculada — e o mapa saía vazio ("nenhuma zona com valor") enquanto a interpolação, olhando as mesmas amostras, desenhava tudo. Agora as duas leem igual. Zona de manejo continua SEM interpolar nada: cada zona recebe o valor absoluto da sua amostra.',
    'COLUNA "CTC EFETIVA" DO LAUDO ERA IMPORTADA COMO CTC pH 7,0. O casamento de cabeçalho aceita nome contido, e "CTC" está dentro de "CTC efetiva" — então a coluna da efetiva era entregue à CTC nominal sempre que viesse ANTES dela na planilha, ou quando o laudo só trouxesse a efetiva. As duas passavam a mostrar o mesmo número, e K%, Ca% e Mg% saíam inflados, porque são calculados dividindo pela CTC (num caso real, Ca% 73,7% no lugar de 47,5%). "CTCe", "CTC ef." e "Capacidade de Troca de Cátions Efetiva" também eram capturadas. Agora a CTC pH 7,0 recusa essas colunas — e, se o laudo não trouxer a CTC nominal, ela fica sem coluna na conferência da importação, em vez de receber calada o número errado. A CTC efetiva segue SEMPRE calculada pela plataforma (Ca+Mg+K+Al), nunca lida de arquivo.',
    'PEDIR CTCe E RECEBER "CTC pH 7,0" NA TELA E NO PDF — corrigido. Quando a conta não tem legenda própria de CTC efetiva, a aba empresta a escala da CTC para conseguir pintar o mapa. Só que a legenda é a fonte da identidade do mapa no app inteiro: o botão da variável mostrava "CTC" (dois botões "CTC" lado a lado), o rodapé dizia "CTC pH 7,0", e o relatório saía com título, unidade, item de capa e NOME DO ARQUIVO de CTC — com os números da CTCe desenhados dentro. Agora o empréstimo é só da escala: identidade, sigla e unidade vêm do catálogo de Variáveis de Análise, e o método "pH 7,0" (que é da CTC nominal) não é mais herdado. O Perfil da Biblioteca também deixou de gravar a legenda emprestada como se fosse escolha do usuário.',
    'FAIXAS DA CTC EFETIVA CORRIGIDAS. A legenda de CTCe criada automaticamente clonava as faixas da CTC pH 7,0 (50/70/140/240 mmolc/dm³) — uma régua para valores que vivem entre 10 e 80, o que jogava o talhão inteiro na classe mais baixa. Ela passa a nascer com as faixas próprias da Fundação ABC (10/20/40/80), e quem já recebeu a clonada tem as faixas trocadas automaticamente, UMA VEZ e só se ainda estiverem intocadas — se você já ajustou algum limite, o seu ajuste manda e nada é alterado.',
    'DIAGNÓSTICO DA IA não usa mais a CTC efetiva no lugar da CTC pH 7,0. Em laudo sem a nominal, ela entrava como se fosse, e o "K na CTC" saía calculado sobre um denominador menor — percentual inflado, sem aviso. Sem CTC pH 7,0 a regra do potássio simplesmente não opina.',
    'A fertilidade por zona não tinha nenhum teste automatizado (o módulo não rodava fora do navegador por causa de duas funções que ninguém chamava). Agora roda: npm run teste:fertzona, com 10 testes que travam o valor absoluto por zona, a igualdade com a leitura da interpolação e a CTCe nunca devolver o número da CTC pH 7,0. Mais 4 testes de cabeçalho em teste:lab e 7 de legenda em teste:legendas.',
  ],
  ...
  '2.112.0': [
    ...
    'ENTROU O FRETE POR INSUMO (R$/t), ao lado do preço: o valor que as contas usam passa a ser PREÇO + FRETE, posto na fazenda. É o que muda tudo no calcário e no gesso, em que o frete costuma pesar mais que o produto e varia de fazenda para fazenda — por isso o frete também pode ser cadastrado no nível da FAZENDA, vencendo o do produtor.',
    'Frete sem preço NÃO vira preço: sem saber quanto custa o produto, o total continua desconhecido em vez de virar só o frete — que passaria por preço e sairia num relatório. E frete zero é zero declarado: serve para tirar um frete herdado do nível de cima.',
    'MUDOU UM VALOR? É SÓ PEDIR O RELATÓRIO DE NOVO. O custo da lavoura deixou de ser lido do que ficou congelado dentro do mapa de colheita: na hora de gerar, vale o que está cadastrado no produtor (ou na fazenda) para aquele ano e cultura. Não é preciso reprocessar mapa nenhum. Sem cadastro no produtor, continua valendo o que foi digitado no mapa, como antes.',
    '23 testes (npm run teste:custos-produtor) travam a herança, o custo por cultura, a soma do frete e os casos em que o valor NÃO deve entrar.',
  ],
  '2.111.0': [
    'PREÇO E CUSTO AGORA PODEM SER DO PRODUTOR, NÃO SÓ DA CASA. Na aba Dados do produtor (e também na da fazenda) entrou o bloco "Custos": preço de cada insumo (R$/t), aplicação (R$/ha) e o custo da lavoura (R$/ha), valendo para aquele produtor no ano — e no semestre, se você quiser separar. Antes, corrigir um preço para um cliente obrigava a mexer no cadastro global, mudando para todo mundo.',
    'A REGRA É A MESMA QUE JÁ VALIA ENTRE EQUAÇÃO E INSUMO: campo em branco HERDA, campo preenchido VENCE, e o mais específico ganha — Biblioteca → produtor → fazenda. O campo mostra em cinza o valor que está valendo e de onde ele vem ("da biblioteca", "do produtor", "da fazenda"), e um link devolve a herança. Nada é copiado da Biblioteca para dentro do produtor quando a tela abre: cópia silenciosa é como preço velho sobrevive a uma atualização de tabela.',
    'ZERO CONTINUA SENDO ZERO DE VERDADE: digitar 0 na aplicação diz "aqui não custa nada" e entra na conta como R$ 0,00 — diferente de deixar em branco, que herda. É a mesma distinção que permite dizer "esta equação não tem frete".',
    'APLICAÇÃO TEM UM PADRÃO: dá para informar um R$/ha único que vale para todo insumo que não tiver o seu próprio, em vez de repetir o mesmo número em cada linha.',
    'O CUSTO DA LAVOURA DEIXA DE SER DIGITADO EM CADA MAPA: informado no produtor (ou na fazenda), ele já entra preenchido no bloco Economia do mapa de colheita, com um aviso dizendo de onde veio — e continua podendo ser mudado só naquele mapa.',
    'OS RELATÓRIOS SEGUEM O VALOR DO PRODUTOR: prescrição/recomendação (o preço puxado ao escolher o insumo), a tabela de equivalentes de fertilizante do relatório de produtividade e o mapa de rentabilidade. Onde não há valor do produtor, tudo continua exatamente como antes, com o número da Biblioteca. 14 testes novos (npm run teste:custos-produtor) travam a herança, a precedência e o zero.',
  ],
  '2.110.0': [
