...
export type Capacidade =
  ...
  | 'zonasUnificar' | 'zonasReclassificar' | 'zonasDividir' | 'zonasSalvar';
...
export const CAPACIDADES: Array<{ id: Capacidade; label: string; curto: string }> = [
  ...
  // Editor Manual de Zonas de Manejo (spec §9) — operações granulares.
  { id: 'zonasUnificar', label: 'Zonas: unificar zonas vizinhas', curto: 'Zonas: unificar' },
  { id: 'zonasReclassificar', label: 'Zonas: reclassificar (trocar a classe)', curto: 'Zonas: reclassificar' },
  { id: 'zonasDividir', label: 'Zonas: dividir (recortar por linha)', curto: 'Zonas: dividir' },
  { id: 'zonasSalvar', label: 'Zonas: salvar edição como nova versão', curto: 'Zonas: salvar' },
];
...
const TODAS = (v: boolean): Caps => ({ cadastro: v, excluirProdutor: v, amostragem: v, importarLaudo: v, fertilidade: v, ndvi: v, recomendacoes: v, biblioteca: v, relatorios: v, zonasUnificar: v, zonasReclassificar: v, zonasDividir: v, zonasSalvar: v });
// Edição de zonas segue quem processa fertilidade (agrônomo edita zonas).
const ZONAS_EDIT = { zonasUnificar: true, zonasReclassificar: true, zonasDividir: true, zonasSalvar: true };
