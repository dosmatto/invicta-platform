...
// Gerador do "Layout Oficial Fertilidade V1" (A4 paisagem) — um elemento por
// página com as profundidades lado a lado. Segue a spec: cabeçalho (logos +
// contexto), mapas (satélite/raster/valores/limite), legenda oficial abaixo,
// estatísticas do RASTER + escala gráfica, rodapé. Abre em nova aba (blob).
...
export async function gerarRelatorioFertilidade(d: DadosRelatorioFert): Promise<void> {
  // ── Validações obrigatórias (spec) ──
  if (!d.profundidades.length) throw new Error('Nenhum mapa interpolado para gerar o relatório.');
  for (const p of d.profundidades) {
    if (!p.rasterPng || !p.bounds) throw new Error(`${p.profundidade}: raster ausente.`);
    if (!p.valores?.features?.length) throw new Error(`${p.profundidade}: sem valores de amostra.`);
    if (!p.stats) throw new Error(`${p.profundidade}: sem estatísticas.`);
  }
  if (!d.legenda) throw new Error('Legenda ausente.');
  if (!d.dataInterpolacao) throw new Error('Data da interpolação ausente.');
  ...
  try {
    ...
    const logoInv = await carregarImg('/images/logo-colorida.png').catch(() => null);
    const logoBranca = await carregarImg('/images/logo-branca.png').catch(() => null);
    const logoCli = d.logoClienteUrl ? await carregarImg(d.logoClienteUrl).catch(() => null) : null;

    const { jsPDF } = await import('jspdf');
    const doc = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });

    // ── CABEÇALHO ──
    if (logoInv) { const h = 15, w = h * (logoInv.naturalWidth / logoInv.naturalHeight); doc.addImage(logoInv, 'PNG', M, 5, w, h); }
    ...
    if (logoCli) { const h = 14, w = Math.min(30, h * (logoCli.naturalWidth / logoCli.naturalHeight)); doc.addImage(logoCli, 'PNG', W - M - 34 + (34 - w) / 2, 8, w, h); }
    ...
    // ── LEGENDA (abaixo dos mapas) ──
    const lgY = mapsY + mapsH + 3;        // ~150
    ...
    // ticks (abaixo) = domínio + limites das classes
    ...
    if (logoBranca) { const h = 5, w = h * (logoBranca.naturalWidth / logoBranca.naturalHeight); doc.addImage(logoBranca, 'PNG', M, H - 7.5, w, h); }
    ...
    // ── saída ──
    const nome = `Fertilidade_${d.talhao}_${d.atributo}`.replace(/[^\w.\-]+/g, '_') + '.pdf';
