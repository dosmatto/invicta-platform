              ...
              <div key={l.id} className="p-2 rounded-lg" style={{ background: '#061525', border: '1px solid #1a3a6b' }}>
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {l.escopo === 'sistema' ? (
                      <>
                        <button onClick={() => onEditar(l.id)} title="Editar esta legenda (ao salvar, ela deixa de ser oficial e passa a ser sua)" className="p-1 rounded" style={{ color: '#93c5fd', background: '#1a3a6b' }}><Edit3 size={10} /></button>
...
function LegendaEditor({ legenda, onClose }: { legenda: Legenda | null; onClose: () => void }) {
  ...
  function salvar() {
    ...
    if (legenda) {
      // Oficial editada vira 'empresa' (mesmo id): é o que permite gravar a
      // alteração sem clonar. Sem isso ela continuaria read-only e o seed poderia
      // repor a versão de fábrica por cima.
      updateLegenda(legenda.id, eraOficial ? { ...form, escopo: 'empresa' } : form);
      // Deixar de ser 'sistema' a faz perder posição para outras oficiais do mesmo
    ...
    <div className="px-3 py-3 space-y-3">
      ...
      {eraOficial && (
        <div className="p-2 rounded text-[10px] leading-relaxed" style={{ background: '#052e2b', color: '#99f6e4', border: '1px solid #0f766e' }}>
          Esta é uma legenda <b>oficial</b>. Você está editando <b>ela mesma</b> — ao salvar, ela passa a ser
          {' '}<b>sua</b> (perde o selo Sistema) e deixa de ser reposta pelo sistema. O cadastro é o mesmo, então
          {' '}os mapas e perfis que já usam esta legenda continuam funcionando, agora com o seu ajuste.
          {' '}Se preferir manter a oficial intacta, volte e use o botão <b>Duplicar</b>.
        </div>
      )}
